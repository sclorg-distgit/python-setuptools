...
BuildRequires:  %{?scl_prefix}python%{python3_pkgversion}-pytest-fixture-config
BuildRequires:  %{?scl_prefix}python%{python3_pkgversion}-pytest-virtualenv
%endif # with tests
%if %{without bootstrap}
BuildRequires:  %{?scl_prefix}python%{python3_pkgversion}-pip
...
Provides: %{?scl_prefix}bundled(python%{1}dist(packaging)) = 16.8
Provides: %{?scl_prefix}bundled(python%{1}dist(pyparsing)) = 2.2.1
Provides: %{?scl_prefix}bundled(python%{1}dist(six)) = 1.10.0
}

%{bundled 3.8}

%if %{with bootstrap}
Provides:       %{?scl_prefix}python%{python3_version}dist(setuptools) = %{version}
%endif
...
%if %{without bootstrap}
%package -n %{?scl_prefix}python%{python3_pkgversion}-setuptools-wheel
Summary:        The setuptools wheel
%{bundled 3.8}

%description -n %{?scl_prefix}python%{python3_pkgversion}-setuptools-wheel
...
%files -n %{?scl_prefix}python%{python3_pkgversion}-setuptools-wheel
%license LICENSE
